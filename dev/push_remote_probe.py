...
SLUG = "adostie3/jed-remote-probe"
TITLE = "JED Remote Probe"  # Kaggle caps titles at 50 chars; slug derived from title on create
...
HOPS = 8
# -- dump remote_agent.py so we can read serialize_history / deserialize_history ----
hdr("STAGE B2 - remote_agent.py source (serialize/deserialize history)")
try:
    import kaggle_evaluation.jed_attack_134815.remote_agent as _ra_mod
    print(open(_ra_mod.__file__, encoding="utf-8", errors="replace").read())
except Exception:
    print("  could not read remote_agent.py:"); traceback.print_exc()

hdr("STAGE C - DIRECT agent vs REMOTE agent (production replay path), hops=8")
# Production replay wraps the agent as RemoteAgent(model_srv.predict); the model server
# runs IN-PROCESS in the gateway (loaded at module level, predict is a direct call), so the
# ONLY difference from a direct SPEC.create_agent(backend) is the serialize_history ->
# predict -> deserialize round-trip. If the forge underperforms in production but not in
# in-process eval_attack, THIS is where it must show up. We run plain + forge through BOTH
# agent factories via eval_attack (hops=8) and compare fire / raw / hops / cost.
if backend is not None and server is not None:
    try:
        ...
        # DIRECT: exactly what our earlier probes used.
        def make_direct():
            return SPEC.create_agent(backend)
        # REMOTE: exactly what jed_attack_gateway._make_agent_factory builds for replay.
        def predict(req):
            return server.predict(req)
        def make_remote():
            return ra.RemoteAgent(predict)

        # sanity: does a RemoteAgent even construct + reset?
        try:
            _r = make_remote(); _r.reset_state()
            print("  RemoteAgent constructed + reset OK:", type(_r).__name__, flush=True)
        except Exception:
            print("  RemoteAgent construction FAILED:"); traceback.print_exc()

        def run_family(fam, tmpl, factory, label):
            ...
            t0 = time.time()
            findings = eval_attack(FixedAttack, 6000.0, factory, "sandbox", options=opts)
            dt = time.time() - t0
            val = len(findings)
            ...
            seq0 = [e.get("name") for e in (findings[0]["trace"].get("tool_events", []) or [])] if findings else []
            # also grab a non-firing example if any (compare to M)
            print(f"  [{label:6s}] {fam:5s}: validated={val}/{M} refire={val/M:.3f} "
                  f"raw={raw:.0f} raw/val={raw/max(1,val):.2f} mean_hops={mean_hops:.2f} "
                  f"per_cand={dt/M:.3f}s seq0={seq0}", flush=True)
            return (label, fam, val/M, raw/max(1,val), mean_hops, dt/M)

        results = []
        for fam, tmpl in FAMILIES.items():
            hdr(f"family={fam}")
            for factory, label in [(make_direct, "direct"), (make_remote, "remote")]:
                try:
                    results.append(run_family(fam, tmpl, factory, label))
                except Exception:
                    print(f"  [{label}] {fam} FAILED:"); traceback.print_exc()

        hdr("SUMMARY  (direct == our probe path; remote == production replay path)")
        print(f"  {'path':7s} {'family':6s} {'refire':>7s} {'raw/val':>8s} {'mean_hops':>10s} {'per_cand':>9s}")
        for label, fam, rf, rper, mh, per in results:
            print(f"  {label:7s} {fam:6s} {rf:7.3f} {rper:8.2f} {mh:10.2f} {per:9.3f}")
        print("\n  If remote-forge refire < direct-forge refire (or mean_hops/cost higher), the")
        print("  serialize/deserialize_history round-trip mangles the Harmony control tokens ->")
        print("  THAT is the v18 forge regression, and the fix lives in how the forge is encoded.")
    except Exception:
        print("  STAGE C FAILED:"); traceback.print_exc()
else:
    print("  SKIPPED: no backend/server")
